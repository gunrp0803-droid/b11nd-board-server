...
public class PostController {
    ...
    // 6. 특정 게시글에 좋아요 누르기 API (토글)
    @PostMapping("/{postId}/likes")
    public ResponseEntity<ApiResponse<String>> likePost(
            @PathVariable Long postId,
            @AuthenticationPrincipal MemberDetails memberDetails) {

        boolean isLiked = postService.likePost(postId, memberDetails.getUserId());
        String message = isLiked ? "좋아요 반영" : "좋아요 취소";
        return ResponseEntity.ok(ApiResponse.ok(message, ResponseKind.VALIDATION_ERROR, message));
    }
}